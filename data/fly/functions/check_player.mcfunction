...
execute as @a[tag=save_fly.can_fly, tag=!save_fly.registered] at @s run summon minecraft:shulker ~ ~-1 ~ {DeathLootTable:"",Invulnerable:true,Tags:["save_fly","save_fly.block","save_fly.secondary"],active_effects:[{id:"minecraft:invisibility",amplifier:1b,duration:999999,show_particles:false}],OnGround:true,NoGravity:true,NoAI:true}
